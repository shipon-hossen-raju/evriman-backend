import { MemoryClaimRequest, UserMemory, VerificationStatus } from "@prisma/client";
import prisma from "../../../shared/prisma";
import emailSender from "../../../shared/emailSender";
import { approvedMailGenerate, rejectedMailGenerate } from "./memoryClaimRequest.mail";

const createIntoDb = async (payload: MemoryClaimRequest) => {
  // Validate the data if necessary

  const result = await prisma.memoryClaimRequest.create({ data: payload });
  return result;
};
...
const updateIntoDb = async (id: string, payload: {status:VerificationStatus}) => {
